...
export function exportarAtaPDF(
  ...
  const titulo = ata.reuniao?.titulo || `Ata de ${format(new Date(ata.recebida_em), "dd/MM/yyyy")}`;
  const dataFormatada = format(new Date(ata.recebida_em), "dd 'de' MMMM 'de' yyyy", { locale: ptBR });
  ...
  let infoX = 18;
  if (ata.tom_geral) {
    const tomText = `Tom: ${tomLabels[ata.tom_geral] || ata.tom_geral}`;
    doc.text(tomText, pageWidth / 2, yPos + 8);
  }
  if (ata.urgencia) {
    const urgText = `Urgência: ${urgenciaLabels[ata.urgencia] || ata.urgencia}`;
    doc.text(urgText, pageWidth - 50, yPos + 8);
  }
  ...
  if (ata.resumo_executivo) {
    ...
    doc.roundedRect(14, yPos, pageWidth - 28, 8, 3, 3, 'FD');
    
    doc.setFontSize(11);
    doc.setFont('helvetica', 'bold');
    doc.setTextColor(...COLORS.primary);
    doc.text('Resumo Executivo', 18, yPos + 6);
    yPos += 12;
    
    doc.setFontSize(9);
    doc.setFont('helvetica', 'normal');
    doc.setTextColor(...COLORS.text);
    const resumoLines = doc.splitTextToSize(ata.resumo_executivo, pageWidth - 36);
  ...
  if (decisoes.length > 0) {
    ...
    const decisoesData = decisoes.map(d => [
      d.descricao,
      d.responsavel || '-',
      d.prazo || '-',
    ]);
  ...
  if (riscos.length > 0) {
    ...
    doc.text(`Riscos (${riscos.length})`, 14, yPos);
    yPos += 6;
    ...
    const riscosData = riscos.map(r => [
      r.descricao,
      severidadeLabels[r.severidade || ''] || r.severidade || '-',
    ]);
    
    autoTable(doc, {
      startY: yPos,
      head: [['Descrição', 'Severidade']],
      body: riscosData,
      headStyles: {
        fillColor: [217, 119, 6],
        textColor: COLORS.white,
  ...
  if (oportunidades.length > 0) {
    ...
    doc.text(`Oportunidades (${oportunidades.length})`, 14, yPos);
    yPos += 6;
    
    const potencialLabels: Record<string, string> = {
      alto: 'Alto',
      medio: 'Médio',
      baixo: 'Baixo',
    };
    
    const oportunidadesData = oportunidades.map(o => [
      o.descricao,
      potencialLabels[o.potencial || ''] || o.potencial || '-',
    ]);
    
    autoTable(doc, {
      startY: yPos,
      head: [['Descrição', 'Potencial']],
      body: oportunidadesData,
      headStyles: {
  ...
  // Conteúdo Completo (Markdown convertido para texto)
  checkNewPage(40);
  ...
  const lines = ata.conteudo_markdown.split('\n');
  doc.setTextColor(...COLORS.text);
  ...
  lines.forEach((line) => {
    ...
    if (trimmed.startsWith('# ')) {
      ...
      doc.setFont('helvetica', 'bold');
      doc.setTextColor(...COLORS.primary);
      const headerLines = doc.splitTextToSize(trimmed.slice(2), pageWidth - 28);
      doc.text(headerLines, 14, yPos);
      yPos += headerLines.length * 5 + 4;
      return;
    }
    if (trimmed.startsWith('## ')) {
      doc.setFontSize(11);
      doc.setFont('helvetica', 'bold');
      doc.setTextColor(...COLORS.primary);
      const headerLines = doc.splitTextToSize(trimmed.slice(3), pageWidth - 28);
      doc.text(headerLines, 14, yPos);
      yPos += headerLines.length * 4 + 3;
      return;
    }
    if (trimmed.startsWith('### ')) {
      doc.setFontSize(10);
      doc.setFont('helvetica', 'bold');
      doc.setTextColor(...COLORS.text);
      const headerLines = doc.splitTextToSize(trimmed.slice(4), pageWidth - 28);
      doc.text(headerLines, 14, yPos);
    ...
    if (trimmed.startsWith('- ') || trimmed.startsWith('* ')) {
      ...
      const itemLines = doc.splitTextToSize(itemText, pageWidth - 36);
    ...
    if (/^\d+\.\s/.test(trimmed)) {
      ...
      doc.setFont('helvetica', 'normal');
      doc.setTextColor(...COLORS.text);
      const itemLines = doc.splitTextToSize(trimmed, pageWidth - 36);
      doc.text(itemLines, 18, yPos);
      yPos += itemLines.length * 4 + 2;
      return;
    }
    ...
    // Table rows (simplified - just output as text)
    if (trimmed.startsWith('|') && trimmed.endsWith('|')) {
      ...
      const tableLines = doc.splitTextToSize(tableText, pageWidth - 28);
      doc.text(tableLines, 14, yPos);
      yPos += tableLines.length * 3 + 1;
      return;
    }
    ...
    const textLines = doc.splitTextToSize(cleanText, pageWidth - 28);
  ...
  const fileName = ata.reuniao?.titulo 
    ? `ata_${ata.reuniao.titulo.replace(/[^a-zA-Z0-9]/g, '_')}_${format(new Date(ata.recebida_em), 'yyyy-MM-dd')}.pdf`
    : `ata_${format(new Date(ata.recebida_em), 'yyyy-MM-dd_HHmm')}.pdf`;

...
interface Ata {
  ...
  link_auditoria: string | null; // Transcrição forense (06_Auditoria)
...
// Padrões para detectar resumos inválidos gerados por erro da IA
          ...
          <table className="w-full text-sm border-collapse">
            <thead>
              <tr className="bg-primary/10">
                {headers.map((cell, i) => (
                  <th key={i} className="border border-border px-3 py-2 text-left font-semibold text-primary">
                    {cell.trim()}
                  </th>
                ))}
              </tr>
      ...
      <DialogContent className="max-w-5xl max-h-[95vh] p-0 overflow-hidden">
            ...
            <div className="flex gap-2">
              {ata.link_auditoria && (
                <Button
                  ...
                  asChild
                >
                  <a href={ata.link_auditoria} target="_blank" rel="noopener noreferrer">
              ...
              {ata.link_drive && (
                <Button
                  ...
                  asChild
                >
                  <a href={ata.link_drive} target="_blank" rel="noopener noreferrer">
        ...
        <ScrollArea className="max-h-[calc(95vh-120px)]">
          <div className="p-6 space-y-6">
            {/* Resumo Executivo */}
            {(() => {
              const resumoOriginal = ata.resumo_executivo;
              let resumoExibir = resumoOriginal || '';
              
              if (isResumoInvalido(resumoOriginal)) {
                resumoExibir = extrairResumoDoMarkdown(ata.conteudo_markdown);
              }
                      ...
                      <div key={r.id} className="p-3 bg-white rounded-lg border border-amber-100">
                        <div className="flex items-start justify-between gap-2">
                          ...
                          {r.severidade && severidadeConfig[r.severidade] && (
                            <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${severidadeConfig[r.severidade].className}`}>
                      ...
                      <div key={o.id} className="p-3 bg-white rounded-lg border border-purple-100">
                        <div className="flex items-start justify-between gap-2">
                          ...
                          {o.potencial && potencialConfig[o.potencial] && (
                            <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${potencialConfig[o.potencial].className}`}>
            ...
            <Card className="border-border bg-muted/30">
              ...
              <CardContent>
                <div className="bg-background rounded-lg p-4 border">
                  {renderMarkdown(ata.conteudo_markdown)}
                </div>
              </CardContent>
            </Card>
          </div>
